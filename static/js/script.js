...
async function checkAdmin(uid) {
    try {
        var r = await fetch('/api/check-admin?user_id=' + uid);
        var d = await r.json();
        isAdmin = d.is_admin;
    } catch (e) {}
}
...
function renderCart() {
    ...
    l.innerHTML = cart.map(function(i) {
        return '<div class="cart-item">' +
            ...
            '<i class="fa-solid fa-xmark rm-btn" onclick="rmCart(\'' + i.id + '\')"></i></div>';
    }).join('');
    ...
    try {
        var r = await fetch('/api/orders', {
            ...
            body: JSON.stringify({
                ...
                payment_method: pay,
                items: cart.map(function(i) { return { id: i.id, name: i.name, quantity: i.quantity, price: i.price }; }),
                subtotal: sub
            })
        });
        var d = await r.json();
...
function renderFavs() {
    ...
    g.innerHTML = fp.map(function(p) {
        return '<div class="card" onclick="openDetail(\'' + p.id + '\')"><button class="fav act"><i class="fa-solid fa-heart"></i></button><img src="' + p.image + '"><div class="info"><div class="name">' + p.name + '</div><div class="price-block"><span class="price">' + fmt(p.price) + ' so\'m</span></div></div></div>';
    }).join('');
}
...
function showView(v) {
    curView = v;
    var views = ['productsView', 'detailView', 'cartView', 'checkoutView', 'favoritesView', 'profileView'];
