...
async function loadProducts() {
    ...
    try {
        ...
        grid.innerHTML = '<div style="text-align:center;padding:40px;grid-column:1/-1;color:#EF4444;">Internet aloqasi yo\'q</div>';
    }
}

function inCart(id) { return cart.some(i => i.id === id); }
function getQty(id) { const i = cart.find(i => i.id === id); return i ? i.quantity : 0; }
...
function addCart(id, q = 1) {
    ...
    if (ex) ex.quantity += q;
    else { const p = products.find(i => i.id === id); if (p) cart.push({...p, quantity: q}); }
    saveCart(); updateBadge(); renderProducts();
}
function chgCart(id, c) {
    const it = cart.find(i => i.id === id);
    if (it) { it.quantity += c; if (it.quantity <= 0) cart = cart.filter(i => i.id !== id); }
    saveCart(); updateBadge(); renderProducts(); if (curView === 'cart') renderCart();
}
function rmCart(id) { cart = cart.filter(i => i.id !== id); saveCart(); updateBadge(); renderProducts(); if (curView === 'cart') renderCart(); }
...
function renderCart() {
    ...
    `).join('');
    document.getElementById('cartTotal').textContent = fmt(cart.reduce((s,i) => s + i.price * i.quantity, 0)) + ' so\'m';
}
...
function openCheckout() {
    ...
    document.getElementById('checkoutItems').innerHTML = cart.map(i => `<div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>${i.name} × ${i.quantity}</span><span>${fmt(i.price*i.quantity)} so'm</span></div>`).join('');
    document.getElementById('checkoutTotal').textContent = fmt(cart.reduce((s,i)=>s+i.price*i.quantity,0)) + ' so\'m';
    showView('checkout');
}
...
async function submitOrder(e) {
    ...
    try {
        const r = await fetch('/api/orders', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:userData?.id, name:n, phone:p, address:a, payment_method:pay, items:cart.map(i=>({id:i.id, name:i.name, quantity:i.quantity, price:i.price})), subtotal:sub})});
        const d = await r.json();
        if (d.success) { cart = []; saveCart(); updateBadge(); renderProducts(); toast('Buyurtma #'+d.order.order_id); showView('products'); }
...
function getGPS() {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
        try {
            const r = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&format=json&accept-language=uz`);
            const d = await r.json();
    ...
    }, err => { toast('GPS aniqlanmadi', 'e'); }, {enableHighAccuracy:true, timeout:10000});
}
function saveAddr() { const a = document.getElementById('addressInput').value.trim(); if (a) document.getElementById('cAddr').textContent = a; closeModal('addressModal'); }
